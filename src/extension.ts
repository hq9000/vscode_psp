...
export function activate(context: vscode.ExtensionContext) {
  ...
  const stopCommand = vscode.commands.registerCommand('vscode-psp.stop',
    ErrorHandler.wrapAsync(async () => {
      ...
      if (state !== 'running') {
        vscode.window.showInformationMessage('PSP: Server is not currently running');
        return;
      }
      ...
      vscode.window.showInformationMessage('PSP: Audio stopped successfully');
    }, 'stopCommand')
  );
  ...
  const showLogsCommand = vscode.commands.registerCommand('vscode-psp.showLogs',
    ErrorHandler.wrapAsync(async () => {
      Logger.info('Show logs command invoked');
      Logger.show();
    }, 'showLogsCommand')
  );

  const showGeneratedCodeCommand = vscode.commands.registerCommand('vscode-psp.showGeneratedCode',
    ErrorHandler.wrapAsync(async () => {
      Logger.info('Show generated code command invoked');
      
      const outputFilePath = ConfigurationManager.getOutputFile();
      
      try {
        const uri = vscode.Uri.file(outputFilePath);
        await vscode.commands.executeCommand('vscode.open', uri);
      } catch (error) {
        vscode.window.showErrorMessage(`PSP: Failed to open ${outputFilePath}`);
      }
    }, 'showGeneratedCodeCommand')
  );

  const openSettingsCommand = vscode.commands.registerCommand('vscode-psp.openSettings',
    ErrorHandler.wrapAsync(async () => {
      Logger.info('Open settings command invoked');
      await vscode.commands.executeCommand('workbench.action.openSettings', '@ext:hq9000.vscode-psp');
    }, 'openSettingsCommand')
  );

  // Add commands to subscriptions for proper cleanup
  context.subscriptions.push(startCommand, stopCommand, restartCommand, checkStatusCommand, runCommand, showLogsCommand, showGeneratedCodeCommand, openSettingsCommand);
}
